...
enum class CountryCode {
    ...
    // DK, DM, DO, DZ, EC, EE, EG, EH, ER, ES, ET, FI, FJ, FK, FM, FO, FR, GA, GB, GD, GE,
    ...
    // SM, SN, SO, SR, SS, ST, SV, SX, SY, SZ, TC, TD, TF, TG, TH, TJ, TK, TL, TM, TN, TO,
    // TR, TT, TV, TW, TZ, UA, UG, UM,
    // US, UY, UZ, VA, VC, VE, VG, VI, VN, VU, WF, WS, YE, YT, ZA, ZM, ZW
    KR {
        override fun offset(timeUnit: TimeUnit): Int {
            val hour = 9
            return when (timeUnit) {
                TimeUnit.SECONDS -> hour * 60 * 60
                TimeUnit.MINUTES -> hour * 60
                else -> hour
            }
        }

        override fun now(): OffsetDateTime {
            val offset = offset(TimeUnit.HOURS)
    ...
    companion object {
        fun of(code: String?): CountryCode = code?.let { valueOf(code) } ?: KR
    }

    abstract fun offset(timeUnit: TimeUnit): Int

    abstract fun now(): OffsetDateTime
}
